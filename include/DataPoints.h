...
template <class type> DataPoints<type> & operator/(DataPoints<type> & dps, double a) {
    for(int i=0;i<dps.getNpoints();i++){
        dps[i]/=a;
    }
    return dps;
}
template <class type> void DataPoints<type>::exportPointsFromSet(string fname){
...
template <class type> void DataPoints<type>::modulo() {
